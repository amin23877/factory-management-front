...
function ItemsDetails({
  ...
  const { data: docs } = useSWR<IDocument[]>(
    activeTab === 0 ? `/document/item/${selectedRow.id}` : null
  );
  const { data: boms } = useSWR<IBom[]>(
    activeTab === 1 ? `/bom?ItemId=${selectedRow.id}` : null
  );
  ...
  const { data: evalSteps } = useSWR(
    activeTab === 4 ? `/engineering/eval/task?ItemId=${selectedRow.id}` : null
  );
  const { data: testSteps } = useSWR(
    activeTab === 5 ? `/engineering/test/task?ItemId=${selectedRow.id}` : null
  );
  ...
  const { data: itemQuotes } = useSWR(
    activeTab === 5 ? `/item/${selectedRow.id}/quote` : null
  );
  const { data: itemSOs } = useSWR(
    activeTab === 6 ? `/item/${selectedRow.id}/so` : null
  );

  const { data: uniteHistory } = useSWR(
    activeTab === 8 ? `/unitehistory` : null
  );
  const { data: services } = useSWR(
    activeTab === 10 ? `/service?ItemId=${selectedRow.id}` : null
  );
  ...
  const { data: notes } = useSWR<INote[]>(
    activeTab === 12 ? `/note/item/${selectedRow.id}` : null
  );
        ...
        <Grid item xs={12}>
          <BasePaper>
            ...
            <Box p={3}>
              ...
              {activeTab === 9 && (
                <SalesReport quotes={itemQuotes} salesOrders={itemSOs || []} />
              )}
              {activeTab === 10 && (
                <BaseDataGrid
                  cols={serviceCols}
